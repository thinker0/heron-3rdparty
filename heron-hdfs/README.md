...
### Record Formats
Record format can be controlled by providing an implementation of the `org.apache.heron.hdfs.format.RecordFormat`
interface:
...
The provided `org.apache.heron.hdfs.format.DelimitedRecordFormat` is capable of producing formats such as CSV and
tab-delimited files.


### File Naming
File naming can be controlled by providing an implementation of the `org.apache.heron.hdfs.format.FileNameFormat`
interface:
...
The provided `org.apache.heron.hdfs.format.DefaultFileNameFormat`  will create file names with the following format:
...
The new provided `org.apache.heron.hdfs.format.SimpleFileNameFormat` and `org.apache.heron.hdfs.trident.format.SimpleFileNameFormat` are more flexible, and the `withName` method support parameters as following:

* $TIME - current time. use `withTimeFormat` to format.
* $NUM - rotation number
* $HOST - local host name
* $PARTITION - partition index (`org.apache.heron.hdfs.trident.format.SimpleFileNameFormat` only)
* $COMPONENT - component id (`org.apache.heron.hdfs.format.SimpleFileNameFormat` only)
* $TASK - task id (`org.apache.heron.hdfs.format.SimpleFileNameFormat` only)
...
to clients reading the data) by implementing the `org.apache.heron.hdfs.sync.SyncPolicy` interface:
...
The `org.apache.heron.hdfs.sync.CountSyncPolicy` class simply triggers a sync after the specified number of tuples have
been processed.

### File Rotation Policies
Similar to sync policies, file rotation policies allow you to control when data files are rotated by providing a
`org.apache.heron.hdfs.rotation.FileRotation` interface:
...
The `org.apache.heron.hdfs.rotation.FileSizeRotationPolicy` implementation allows you to trigger file rotation when
...
The `org.apache.heron.hdfs.bolt.SequenceFileBolt` class allows you to write storm data to HDFS sequence files:
...
The `SequenceFileBolt` requires that you provide a `org.apache.heron.hdfs.bolt.format.SequenceFormat` that maps tuples to
key/value pairs:
...
The `org.apache.heron.hdfs.bolt.AvroGenericRecordBolt` class allows you to write Avro objects directly to HDFS:
...
Please see the javadoc for classes in org.apache.heron.hdfs.avro for information about using the built-in options or
creating your own.
...
```
nimbus.autocredential.plugins.classes : ["org.apache.heron.hdfs.security.AutoHDFS"]
nimbus.credential.renewers.classes : ["org.apache.heron.hdfs.security.AutoHDFS"]
...
```
topology.auto-credentials :["org.apache.heron.hdfs.common.security.AutoHDFS"]
```
...
|----------------------------|--------------------------------------|-------------|-------------|
| **.setReaderType()**       |~~hdfsspout.reader.type~~             |             | Determines which file reader to use. Set to 'seq' for reading sequence files or 'text' for text files. Set to a fully qualified class name if using a custom file reader class (that implements interface org.apache.heron.hdfs.spout.FileReader)|
